...
## Parsing PDF Content
To parse exactly PDF content, we need to read all plain PDF information. However, beside some pure control data, all streams in PDF are usually in compressed format. That means we need to decompress them to get the pure text. The compress algorithm is given clearly in  stream description, for example:
